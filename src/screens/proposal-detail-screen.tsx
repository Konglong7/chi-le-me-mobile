...
export function ProposalDetailScreen() {
  ...
  const initialVote =
    proposal.voteOptions.find((option) => option.voterNicknames.includes(currentNickname))?.id ?? proposal.voteOptions[0]?.id;
  ...
  const shareProposal = async () => {
    ...
    if (typeof navigator !== 'undefined' && 'share' in navigator) {
      try {
        await navigator.share({ title: proposal.title, text });
        return;
      } catch {
        // noop
      }
  ...
  return (
    <AppShell>
      <div className="absolute left-0 top-0 z-40 flex h-[72px] w-full items-end border-b border-slate-100 bg-white/90 px-4 pb-3 shadow-sm backdrop-blur">
        <div className="flex w-full items-center justify-between">
          ...
          <button type="button" className="p-2 text-lg" aria-label="分享提案" onClick={() => void shareProposal()}>
            <FaShareNodes />
          </button>
        </div>
      </div>

      <ScreenScroller className="bg-slate-50 px-0 pb-[170px] pt-[72px]">
        <div className="relative mb-4 rounded-b-3xl bg-white p-5 pt-6 shadow-sm">
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 text-sm text-slate-500">备注：{proposal.remark ?? '暂无备注'}</div>
        </div>

        {proposal.voteEnabled ? (
            ...
            <SectionCard className="p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={isCreator}
                  className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                >
                  {isCreator ? '发起人不可退出' : joined ? '退出组队' : '我要去'}
                </button>
              </div>
              <div className="flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
                {proposal.teamMembers.map((member) => (
                  <div key={member.nickname} className="flex flex-col items-center gap-1">
                    <Avatar seed={member.avatarSeed} className={`h-10 w-10 ${member.isCreator ? 'border-2 border-yellow-400 p-0.5' : ''}`} />
                ...
                {proposal.teamMembers.length < proposal.maxPeople ? (
                  <div className="flex flex-col items-center gap-1">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full border border-dashed border-slate-300 text-slate-300">+</div>
                  ...
                  <div key={chat.id} className={`flex items-start gap-2 ${mine ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div
                      className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${
                        mine ? 'rounded-tl-xl rounded-bl-xl bg-theme-500 text-white shadow-sm' : 'rounded-tr-xl rounded-br-xl bg-slate-100 text-slate-800'
                      }`}
                    >
                      {chat.content}
                    </div>
      ...
      <div className="absolute bottom-[84px] left-0 z-50 flex w-full items-center gap-2 border-t border-slate-100 bg-white p-3">
        <input
          value={message}
