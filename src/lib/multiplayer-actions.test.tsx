...
describe('remote multiplayer actions', () => {
  ...
  it('does not fabricate a local proposal when the remote create request fails', async () => {
    const user = userEvent.setup();
    ...
    await user.click(screen.getByRole('button', { name: '发起提案' }));
    await user.type(screen.getByPlaceholderText('例如：今晚去吃什么？'), '失败后不能本地伪造');
    await user.click(screen.getByRole('button', { name: '发布提案' }));
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: '发起干饭提案' })).toBeInTheDocument();
      expect(screen.queryByText('失败后不能本地伪造')).not.toBeInTheDocument();
      expect(screen.getByRole('alert')).toHaveTextContent('提案发布失败，请稍后重试');
    });
  });

  it('keeps the draft message when remote sending fails', async () => {
    const user = userEvent.setup();
    ...
    await user.click(screen.getByRole('button', { name: '查看详情 远程提案：今晚吃烤肉？' }));
    await user.type(screen.getByPlaceholderText('说点什么...'), '这条消息不能丢');
    ...
    expect(screen.getByPlaceholderText('说点什么...')).toHaveValue('这条消息不能丢');
    expect(screen.getByRole('alert')).toHaveTextContent('消息发送失败，请稍后重试');
  });
});
